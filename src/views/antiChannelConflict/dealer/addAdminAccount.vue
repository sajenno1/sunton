<template>
  <div>
    <el-dialog title="添加经销商微信端登录账户" :visible.sync="dialogAddVisible" width="40%">
      <el-form :model="form" style="padding-right:5%;">
        ...
        <el-form-item label="用户名" :label-width="formLabelWidth">
          <el-input type="userName" v-model="form.userName" auto-complete="off" size="small"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    create () {
      this.$http.post(api.addDealerAdminAccount, this.form).then(res => {
    ...
    selectedUser (index, row) {
      ...
      this.form.displayName = this.row.name + '(' + row.nickName + ')'
    }
  }
}
</script>
