<template>
  <div>
    <el-dialog title="添加经销商" :visible.sync="show" width="50%">
      <el-form :model="form" style="padding-right:5%;">
        ...
        <el-form-item label="设置密码" :label-width="formLabelWidth">
          <el-input type="password" v-model="form.password" auto-complete="off" size="small"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    create () {
      this.$http.post(api.addDealer, this.form).then(res => {
    ...
    async getDealerLevel () {
      this.$http.post(api.dealerLevelAllList).then(res => {
        ...
        if (this.optionsDealerLevel.length > 0) this.form.dealerLevelId = this.optionsDealerLevel[0].id
      })
    },
    async getArea () {
