<template>
  <div>
    <el-dialog title="编辑经销商" :visible.sync="show" width="50%">
      <el-form :model="form" style="padding-right:5%;">
        ...
        <el-form-item label="授信额度" :label-width="formLabelWidth">
          <el-input v-model="form.creditLimit" auto-complete="off" size="small"></el-input>
        </el-form-item>
        <el-form-item label="越级发货" :label-width="formLabelWidth">
        ...
        <el-form-item label="是否为门店" :label-width="formLabelWidth">
          <el-checkbox v-model="form.isSellStore">是</el-checkbox>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    update () {
      this.$http.post(api.editDealer, this.form).then(res => {
    ...
    getBalances (ids) {
      var _this = this
      this.$http.post(api.mallDealerAccountBalanceList, {ids: ids}).then(res => {
        // this.$notify.success({ title: '成功', message: res.data })
        // for (var item of this.list) {
        // item.balance = '不' + res.data[item.id].toString()
        _this.balance = res.data[ids[0]]
        // console.log(item)
        // }
