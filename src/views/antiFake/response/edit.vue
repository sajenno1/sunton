<template>
  <div>
    <el-dialog title="编辑防伪回复" :visible.sync="show" width="50%">
...
export default {
  ...
  methods: {
    ...
    getData (id) {
      this.$http.post(api.antiFakeResponseInfo, { id: id }).then(res => {
        this.form = res.data
      })
    },
    confirm () {
    ...
    loadIssuerCodeRecords (id) {
      var _this = this
      this.$http.post(api.loadUnRelationshipIssuerRecords, { id: id }).then(res => {
        _this.issuerCodeRecords = res.data
      })
    },
    update () {
      this.$http.post(api.editAntiFakeResponse, this.form).then(res => {
