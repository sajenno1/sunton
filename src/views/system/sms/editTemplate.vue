<template>
  <div>
    <el-dialog title="编辑短信模板" :visible.sync="show" width="40%" append-to-body>
...
export default {
  ...
  methods: {
    open (row) {
      this.form = {}
      this.show = true
      this.getTemplateKeys().then(() => {
        this.getData()
      })
    },
    getTemplateKeys () {
      return this.$http.post(api.templateKeys).then(res => {
        this.templateKeys = res.data
      })
    },
    getData (id) {
      this.$http.post(api.smsTemplateInfo, { id: id }).then(res => {
        this.form = res.data
      })
    },
    update () {
      this.$http.post(api.editSmsTemplate, this.form).then(res => {
        this.$emit('getList')
        this.show = false
