...
import productPropertyValue from '../replenishProductPropertyValue/list'
export default {
  ...
  methods: {
    getList () {
      this.$http.post(api.replenishProductPropertyList, {
    ...
    handleDelete (index, row) {
      this.$confirm('删除该属性值将清空该产品属性下的属性值，确定删除吗？').then(() => {
        this.$http.post(api.delReplenishProductProperty, { id: row.id }).then(res => {
          this.getList()
