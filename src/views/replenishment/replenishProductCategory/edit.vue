...
export default {
  name: 'replenishment-replenishProductCategory-add',
  data () {
    return {
  ...
  methods: {
    ...
    getData (id) {
      this.$http.post(api.replenishProductCategoryInfo, { id: id }).then(res => {
        this.form = res.data
      })
    },
    ...
    update () {
      this.$http.post(api.editReplenishProductCategory, this.form).then(res => {
        this.$emit('getList')
        this.show = false
