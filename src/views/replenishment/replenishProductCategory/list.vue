...
export default {
  ...
  methods: {
    getTree () {
      this.$http.post(api.replenishProductCategoryTree).then(res => {
        this.tree = res.data
      })
    },
    refresh () {
    ...
    getList () {
      this.$http.post(api.replenishProductCategoryList, { id: this.selected.id }).then(res => {
        this.list = res.data
    ...
    handleDelete (index, row) {
      this.$confirm('确定删除该商品分类吗？')
        .then(() => {
          this.$http.post(api.delReplenishProductCategory, { id: row.id }).then(res => {
            this.getList()
    ...
    saveSort () {
      ...
      this.$http.post(api.setReplenishProductCategorySort, { sort: sortList }).then(res => {
