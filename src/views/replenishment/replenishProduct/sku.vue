...
export default {
  name: 'replenishment-replenishProduct-sku',
  components: { addSku },
  data () {
    return {
      show: false,
  ...
  methods: {
    ...
    async getList () {
      await this.$http.post(api.replenishProductSkuList, { id: this.row.id }).then(res => {
    ...
    handleDelete (index, row) {
      this.$confirm('确定删除该Sku吗？').then(() => {
        this.$http.post(api.delReplenishProductSku, { id: row.id }).then(res => {
          this.getList()
    ...
    handleSave (index, row) {
      this.$http.post(api.editReplenishProductSku, row).then(res => {
