  ...
  <div>
    <el-dialog title="添加产品" :visible.sync="show" width="60%">
      <el-form :model="form" style="padding-right:5%;">
        ...
        <el-form-item label="销售属性分类" :label-width="formLabelWidth">
          <el-select v-model="form.propertyCategoryId" placeholder="请选择销售属性分类">
            <el-option
...
export default {
  name: 'replenishment-replenishProduct-add',
  data () {
    return {
  ...
  methods: {
    open () {
      ...
      var _this = this
      setTimeout(() => {
        _this.ue = window.UE.createEditor('editor1', {
          BaseUrl: '',
          UEDITOR_HOME_URL: 'static/utf8-net/'
        })
      })
    ...
    getBrandList () {
      this.$http.post(api.replenishProductBrandList, this.pageParams).then(res => {
        this.brandList = res.data.rows
      })
    },
    getPropertyCategoryList () {
      this.$http.post(api.replenishProductPropertyCategoryList, this.pageParams).then(res => {
    ...
    getCategoryList () {
      this.$http.post(api.replenishProductCategoryCascader).then(res => {
