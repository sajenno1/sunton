...
export default {
  name: 'replenishment-replenishProductPropertyCategory-editValues',
  data () {
    return {
      show: false,
      form: {},
  ...
  methods: {
    ...
    getData (id) {
      this.$http.post(api.replenishProductPropertyValueInfo, { id: id }).then(res => {
        this.form = res.data
      })
    },
    update () {
      this.$http.post(api.editReplenishProductPropertyValue, this.form).then(res => {
        this.$emit('getList')
        this.show = false
