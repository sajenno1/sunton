...
export default {
  ...
  methods: {
    open (row) {
      ...
      setTimeout(() => {
        ...
        _this.ue.addListener('ready', function () {
          _this.ue.setContent(_this.form.newsContent || '')
        })
      })
    },
    getInfo (id) {
      this.$http.post(api.weChatExcellentNewsInfo, { id: id }).then(res => {
        this.form = res.data
    ...
    update () {
      this.form.newsContent = window.UE.getEditor('editor1').getContent()
      this.$http.post(api.weChatExcellentNewsEdit, this.form).then(res => {
        this.$emit('getList')
        this.show = false
