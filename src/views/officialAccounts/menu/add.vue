<template>
  <div>
    <el-dialog title="添加菜单" :visible.sync="show" width="40%">
      <el-form :model="form" label-width="120px" style="padding-right:5%;">

        <el-form-item label="父级菜单">
          <el-select clearable v-model="form.parentId" placeholder="放空为顶级菜单">
      ...
      <div>
        <selected-news ref="selectedNews" @selected="handleNewsSelected"/>

        <selected-image ref="selectedImage" @selected="handleImageSelected"/>

        <selected-others ref="selectedOthers" @selected="handleOthersSelected"/>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  name: 'officialAccounts-menu-add',
  ...
  methods: {
    ...
    create () {
      this.$http.post(api.addOfficialAccountMenu, this.form).then(res => {
        this.show = false
        this.$emit('getList')
