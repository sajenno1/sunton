...
<div>
    <el-card>
      <div slot="header">
        <span v-on:click="reRreshData">客户统计报表</span>
      </div>
    ...
    <div class="baobiao">
      ...
      <el-tabs v-model="degreeActiveName" v-on:tab-click="degreeTabClick">
        <el-tab-pane label="客户" name="dealer" class="pad3"></el-tab-pane>
        <el-tab-pane label="级别" name="level"></el-tab-pane>
      ...
      <div class="choose-btn">
        <div>
          <el-button-group>
            ...
            <el-button size='small' v-on:click="changeStatType(2)" v-bind:class="{active:statType==2}">订单数</el-button>
            <!-- <el-button size='small' v-on:click="changeStatType(2)" v-bind:class="{active:statType==2}">款数</el-button> -->
          </el-button-group>
        </div>
        <div>
          <el-button-group>
      ...
      <el-table v-bind:data="reportDataRows" style="width: 100%;text-align:center;margin-top:1%" show-summary>
        <el-table-column v-bind:label="degreeActiveName=='dealer'?'客户':degreeActiveName=='level'?'级别':'区域'" align='center' width='180'>
        <!-- <el-table-column v-bind:label="formLabelWidth" align='center' width='180'> -->
          <template slot-scope="scope">
            {{degreeActiveName=='dealer'?scope.row.dealerName:''}}
            {{degreeActiveName=='level'?scope.row.levelName:''}}
            {{degreeActiveName=='area'?scope.row.areaName:''}}
          </template>
        </el-table-column>
        <el-table-column prop="orderCount" label="订货单数" align='center'>
        </el-table-column>
        <!-- <el-table-column prop="dealerCount" label="订货款数" align='center'>
        </el-table-column> -->
...
// },
    degreeTabClick (tab, event) {
      ...
      // console.log(this.degreeActiveName)
      this.buildReport()
    },
    pickerSearch () {
      this.searchReports()
    ...
    init () {
      ...
      this.reportDataSet = {'dealer': [], 'level': [], 'area': []}
      this.searchReports()
    },
    searchReports () {
      ...
      // }
      this.imageType = 0
      this.degreeActiveName = 'dealer'
      this.statType = 0
      var _this = this

      _this.$http.post(api.SellDealerReportByDealer, {
        createDate1: createDate1,
        createDate2: createDate2
      }).then(res => {
        _this.reportDataSet['dealer'] = res.data.rows
        _this.buildReport()
      })
      setTimeout(function () {
        _this.$http.post(api.SellDealerReportByLevel, {
          createDate1: createDate1,
          createDate2: createDate2
        }).then(res => {
          _this.reportDataSet['level'] = res.data.rows
          _this.buildReport()
        })
      }, 500)
      setTimeout(function () {
        _this.$http.post(api.SellDealerReportByArea, {
          createDate1: createDate1,
          createDate2: createDate2
        }).then(res => {
          _this.reportDataSet['area'] = res.data.rows
          _this.buildReport()
        })
      }, 800)
    },
    buildReport () {
      ...
      reportDataRows.forEach(function (item) {
        ...
        d2Arr[2] += item.orderCount

        if (_this.degreeActiveName === 'dealer') {
          nArr.push(item.dealerName)
          amArr.push({value: item.amountSum, name: item.dealerName})
          qArr.push({value: item.quantitySum, name: item.dealerName})
          orArr.push({value: item.orderCount, name: item.dealerName})
          // deArr.push({value: item.dealerCount, name: item.dealerName})
        } else if (_this.degreeActiveName === 'level') {
          nArr.push(item.levelName)
          amArr.push({value: item.amountSum, name: item.levelName})
          qArr.push({value: item.quantitySum, name: item.levelName})
          orArr.push({value: item.orderCount, name: item.levelName})
          // deArr.push({value: item.dealerCount, name: item.seriesName})
        } else if (_this.degreeActiveName === 'area') {
          nArr.push(item.areaName)
          amArr.push({value: item.amountSum, name: item.areaName})
          qArr.push({value: item.quantitySum, name: item.areaName})
          orArr.push({value: item.orderCount, name: item.areaName})
      ...
      this.pieData.Data3 = [qArr, amArr, orArr]
      this.pieData.Data4 = ['数量', '金额', '订单数']
    ...
    getOrderCharts () {
      ...
      var report1Data5 = this.statType === 0 ? '数量' : this.statType === 1 ? '金额' : '订单数'
