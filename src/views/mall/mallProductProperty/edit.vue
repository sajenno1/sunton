...
export default {
  name: 'mall-mallProductProperty-edit',
  data () {
    return {
      show: false,
      form: {},
  ...
  methods: {
    ...
    getData (id) {
      this.$http.post(api.mallProductPropertyInfo, { id: id }).then(res => {
        this.form = res.data
      })
    },
    update () {
      this.$http.post(api.editMallProductProperty, this.form).then(res => {
        this.$emit('getList')
        this.show = false
