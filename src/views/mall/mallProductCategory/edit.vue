...
export default {
  name: 'mall-mallProductCategory-add',
  data () {
    return {
  ...
  methods: {
    ...
    getData (id) {
      this.$http.post(api.mallProductCategoryInfo, { id: id }).then(res => {
        this.form = res.data
      })
    },
    ...
    update () {
      this.$http.post(api.editMallProductCategory, this.form).then(res => {
        this.$emit('getList')
        this.show = false
