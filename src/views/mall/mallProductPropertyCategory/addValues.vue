...
export default {
  name: 'mall-mallProductPropertyCategory-addValues',
  data () {
    return {
      show: false,
      form: {},
  ...
  methods: {
    ...
    getPropertys () {
      this.$http.post(api.getMallProductPropertyList, { isSaleProp: this.isSaleProp }).then(res => {
        this.propertys = res.data
      })
    },
    create () {
      this.$http.post(api.relationMallPropertyValues, this.form).then(res => {
        this.$emit('getList')
        this.show = false
    ...
    getPropertyValues (data) {
      return this.$http.post(api.mallProductPropertyValueList, {
        propertyId: data,
        page: 1,
        pageSize: 1000
      }).then(res => {
    ...
    getProductValueIds (data) {
      this.$http.post(api.getMallProductPropertyValuesByRootValue, {
