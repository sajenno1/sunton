...
export default {
  name: 'mall-mallProductPropertyCategory-values',
  ...
  methods: {
    ...
    getList () {
      this.$http.post(api.getMallPropertyVaules, { id: this.row.id, isSaleProp: this.isSaleProp }).then(res => {
        this.list = res.data
    ...
    handleDelete (index, row) {
      this.$confirm('确定删除该商品属性值吗？')
        .then(() => {
          this.$http.post(api.delRelationMallPropertyValue, { id: row.id }).then(res => {
            this.getList()
