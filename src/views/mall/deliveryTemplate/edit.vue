  ...
  <div>
    <el-dialog title="添加运费模板" :visible.sync="show" width="60%">
      <el-form :model="form" style="padding-right:5%;">
        ...
        <!-- <el-form-item label="模板备注" :label-width="formLabelWidth">
...
export default {
  name: 'mall-deliveryTemplate-add',
  data () {
    return {
      show: false,
      form: {},
  ...
  methods: {
    ...
    getData (id) {
      this.$http.post(api.deliveryTemplateInfo, { id: id }).then(res => {
        this.form = res.data
      })
    },
    loadAreas () {
    ...
    update () {
      ...
      this.$http.post(api.editDeliveryTemplate, this.form).then(res => {
        this.$emit('getList')
        this.show = false
