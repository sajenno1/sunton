...
export default {
  ...
  methods: {
    ...
    getList () {
      this.$http.post(api.getMallPropertyVaules, {
        id: this.row.productPropertyCategoryId,
        isSaleProp: true
      }).then(res => {
        // let set = new Set()
        // this.valueIds.map(x => {
    ...
    create () {
      ...
      this.$http.post(api.addMallProductSku, form).then(res => {
        this.$emit('getList')
        this.show = false
