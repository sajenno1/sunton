...
export default {
  ...
  methods: {
    async open (id) {
      ...
      await _this.getBrandList()
      await _this.getPropertyCategoryList()
      await _this.getDeliveryTemplateList()
      await _this.getCategoryList()
      await _this.getTopCategory(_this.form.categoryId)
      _this.ue = window.UE.createEditor('editor', {
        BaseUrl: '',
        UEDITOR_HOME_URL: 'static/utf8-net/'
      })
      _this.ue.addListener('ready', function () {
        _this.ue.setContent(_this.form.description || '')
      })
    },
    async getInfo (id) {
      await this.$http.post(api.mallProductInfo, { id: id }).then(res => {
        this.form = res.data
    ...
    async getBrandList () {
      await this.$http.post(api.mallBrandList, this.pageParams).then(res => {
        this.brandList = res.data.rows
      })
    },
    async getPropertyCategoryList () {
      await this.$http.post(api.mallProductPropertyCategoryList, this.pageParams).then(res => {
    ...
    async getCategoryList () {
      await this.$http.post(api.mallProductCategoryCascader).then(res => {
        this.categoryList = res.data
      })
    },
    async getTopCategory (id) {
      await this.$http.post(api.getTopMallCategory, { id: id }).then(res => {
    ...
    update () {
      ...
      this.form.description = window.UE.getEditor('editor').getContent()
      this.$http.post(api.editMallProduct, this.form).then(res => {
        this.$emit('getList')
        this.show = false
