<template>
  <div>
    <el-dialog title="编辑图片" :visible.sync="show" width="40%" append-to-body>
...
export default {
  ...
  methods: {
    ...
    getData () {
      this.$http.post(api.advertItemInfo, { id: this.row.id }).then(res => {
        this.form = res.data
      })
    },
    update () {
      this.$http.post(api.editAdvertItem, this.form).then(res => {
        this.$emit('getList')
        this.show = false
