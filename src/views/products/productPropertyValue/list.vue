...
import addProductPropertyValue from './add'
import editProductPropertyValue from './edit'
export default {
  name: 'products-productPropertyValue-list',
  components: { addProductPropertyValue, editProductPropertyValue },
  data () {
    return {
      show: false,
      list: [],
  ...
  methods: {
    ...
    getList () {
      this.$http.post(api.productPropertyValueList, { propertyId: this.row.id }).then(res => {
    ...
    saveSort () {
      ...
      this.$http.post(api.setProductPropertyValueSort, { sort: sortList }).then(res => {
    ...
    handleDelete (index, row) {
      this.$confirm('确定删除该产品属性值吗？')
        .then(() => {
          this.$http.post(api.delProductPropertyValue, { id: row.id }).then(res => {
            this.getList()
