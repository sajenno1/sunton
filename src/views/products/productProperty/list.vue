...
import productPropertyValue from '../productPropertyValue/list'
export default {
  ...
  methods: {
    getList () {
      this.$http.post(api.productPropertyList, {
    ...
    handleDelete (index, row) {
      this.$confirm('删除该属性值将清空该产品属性下的属性值，确定删除吗？').then(() => {
        this.$http.post(api.delProductProperty, { id: row.id }).then(res => {
          this.getList()
