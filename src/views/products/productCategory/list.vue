            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <img v-if="scope.row.photo" :src="scope.row.photo" :alt="scope.row.name">
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
    getTree () {
      this.$http.post(api.productCategoryTree).then(res => {
        this.tree = res.data
      })
    },
    refresh () {
    ...
    getList () {
      this.$http.post(api.productCategoryList, { id: this.selected.id }).then(res => {
        this.list = res.data
    ...
    handleDelete (index, row) {
      this.$confirm('确定删除该产品分类吗？')
        .then(() => {
          this.$http.post(api.delProductCategory, { id: row.id }).then(res => {
            this.refresh()
    ...
    saveSort () {
      ...
      this.$http.post(api.setProductCategorySort, { sort: sortList }).then(res => {
