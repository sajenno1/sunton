<template>
  <div>
    <el-dialog title="产品SKU" :visible.sync="show" width="30%">
        ...
        <el-table
            ...
            <template slot-scope="scope">
              <div>
                <!--<span v-if="scope.row.isEdit">
                  <el-input v-model="scope.row.productSkuCode"></el-input>
                </span>-->
                <span>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div>
                <span v-if="scope.row.isEdit">
                  <el-input v-model="scope.row.tradePrice"></el-input>
                </span>
                <span v-else>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div>
                <span v-if="scope.row.isEdit">
                  <el-input v-model="scope.row.upperLimitInventory"></el-input>
                </span>
                <span v-else>
                  {{ scope.row.upperLimitInventory }}
                </span>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div>
                <span v-if="scope.row.isEdit">
                  <el-input v-model="scope.row.lowerLimitInventory"></el-input>
                </span>
                <span v-else>
                  {{ scope.row.lowerLimitInventory }}
                </span>
              </div>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getList () {
      await this.$http.post(api.productSkuList, { id: this.row.id }).then(res => {
    ...
    handleDelete (index, row) {
      this.$confirm('确定删除该Sku吗？').then(() => {
        this.$http.post(api.delSku, { id: row.id }).then(res => {
          this.getList()
    ...
    handleSave (index, row) {
      this.$http.post(api.editSku, row).then(res => {
